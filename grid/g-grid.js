...
const ggridEmitter = new EventEmitter()
...
function sendEmail(message,subject){
    try{
        ...
        if (lastSentTimestamps[subjectKey] && now - lastSentTimestamps[subjectKey] < THROTTLE_INTERVAL_MS) {
            console.log(`GAYATRI GRID BOT : Throttled: Email with subject "${subject}" was sent recently.`);
            return;
        }
...
const KEY           =   process.env.G_GRID_WEB_KEY
const SECRET        =   process.env.G_GRID_WEB_SECRET 
        ...
        try{
            ...
            if (message.type === 'success' && message.message === 'Authenticated') {
                ...
                if(total_error_count > 3) {   
                    ...
                    is_live = false
                    fs.writeFileSync('./grid/g-orderInfo.json', '', 'utf8')
...
(async function() {
    ...
    is_live = (fs.statSync('./grid/g-orderInfo.json').size != 0)?true:false
    if(is_live){
        wsConnect()
        let order_data = fs.readFileSync('./grid/g-orderInfo.json', 'utf8')
...
async function updateOrderInfo(content){
    fs.writeFile('./grid/g-orderInfo.json', content, (error) => {
        if (error) {
...
async function socketEventInfo(current_price){
    ...
    is_live = (fs.statSync('./grid/g-orderInfo.json').size != 0)?true:false
    if(is_live){
        order_data = fs.readFileSync('./grid/g-orderInfo.json', 'utf8')
        order_data = JSON.parse(order_data) 
    } 
    //let current_trend = await classifyLastCandle()
    let current_trend = "Neutral"
    ggridEmitter.emit("grid_trade_info", {
...
ggridEmitter.on("g_grid_start", async () => { 
    await setRangeLimitOrder()
    is_live = true 
    wsConnect()
...
ggridEmitter.on("g_grid_stop", async () => { 
    total_error_count = 0
    await cancelAllOpenOrder() 
    fs.writeFileSync('./grid/g-orderInfo.json', '', 'utf8')
